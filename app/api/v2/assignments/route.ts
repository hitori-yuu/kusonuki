...
import { CreateAssignment, findAssignmentsByDate, findAssignmentsByRange, getAllAssignments } from "@/lib/server/actions";
...
export async function GET(req: NextRequest) {
	...
	if (academicYear && grade && className) {
		if (dateRange) {
			const quizData = await findAssignmentsByRange(parseInt(academicYear), parseInt(grade), className, parseInt(dateRange)) as AssignmentData[];
			return NextResponse.json(quizData);
		} else if (dueDate) {
			const quizData = await findAssignmentsByDate(parseInt(academicYear), parseInt(grade), className, new Date(dueDate)) as AssignmentData[];
			return NextResponse.json(quizData);
		} else {
