...
import { CreateAssignment, CreateQuiz, findQuizByDate, findQuizByRange, getAllAssignments, getAllQuiz } from "@/lib/server/actions";
...
export async function GET(req: NextRequest) {
    ...
    if (academicYear && grade && className) {
        if (dateRange) {
            const quizData = await findQuizByRange(parseInt(academicYear), parseInt(grade), className, parseInt(dateRange)) as QuizData[];
            return NextResponse.json(quizData);
        } else if (testDate) {
            const quizData = await findQuizByDate(parseInt(academicYear), parseInt(grade), className, new Date(testDate)) as QuizData[];
            return NextResponse.json(quizData);
        } else {
