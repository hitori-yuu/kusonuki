...
export async function GET(req: NextRequest) {
    ...
    if (grade && className && number) {
        if (fullName) {
            const studentData = await findStudentByFullName(fullName, parseInt(grade), className, parseInt(number)) as StudentData;
            return NextResponse.json(studentData);
        } else if (academicYear) {
            const studentData = await findStudentByHistory(parseInt(academicYear), parseInt(grade), className, parseInt(number)) as StudentData;
            return NextResponse.json(studentData);
        } else {
            const studentData = await findStudentByCurrent(parseInt(grade), className, parseInt(number)) as StudentData;
            return NextResponse.json(studentData);
        }
    }
